# WindowEnumeration_Integration.Tests.ps1
# Integration tests for WindowEnumerationAPI P/Invoke methods
# Tests real Win32 API calls, requiring a live desktop environment
...
Describe 'WindowEnumerationAPI Integration Tests' -Tag 'Integration' {
    Context 'ShowWindow Functional Tests' {
        BeforeAll {
            $script:showWindowHandle = & ${script:Get-TestRunnerWindowHandle}
        }
        It 'Should call ShowWindow without throwing' -Skip:(-not [Environment]::UserInteractive -or [bool]$env:CI -or [bool]$env:TF_BUILD -or [bool]$env:GITHUB_ACTIONS) {
            $hWnd = $script:showWindowHandle
            $SW_MINIMIZE = 2
            $SW_RESTORE = 9
            {
                [LastWarAutoScreenshot.WindowEnumerationAPI]::ShowWindow($hWnd, $SW_MINIMIZE) | Out-Null
                [LastWarAutoScreenshot.WindowEnumerationAPI]::ShowWindow($hWnd, $SW_RESTORE) | Out-Null
            } | Should -Not -Throw
        }
    }
