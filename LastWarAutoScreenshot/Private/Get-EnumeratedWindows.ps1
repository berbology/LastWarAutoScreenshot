function Get-EnumeratedWindows {
    ...
    begin {
        ...
        # Removed legacy type check and dot-sourcing for WindowEnumeration_TypeDefinition.ps1; types are loaded by module import
    ...
    process {
        ...
        # Must be stored in variable to prevent premature garbage collection
            $enumCallback = [LastWarAutoScreenshot.EnumWindowsProc] {
            param($hwnd, $lParam)
            ...
            try {
                # Check if window is visible
                    $isVisible = [LastWarAutoScreenshot.WindowEnumerationAPI]::IsWindowVisible($hwnd)
                if (-not $isVisible) {
                ...
                # Check if window is minimized
                    $isMinimized = [LastWarAutoScreenshot.WindowEnumerationAPI]::IsIconic($hwnd)
                
                # Get window title
                    $titleLength = [LastWarAutoScreenshot.WindowEnumerationAPI]::GetWindowTextLength($hwnd)
                $windowTitle = ''
                
                if ($titleLength -gt 0) {
                    $stringBuilder = [System.Text.StringBuilder]::new($titleLength + 1)
                        $result = [LastWarAutoScreenshot.WindowEnumerationAPI]::GetWindowText($hwnd, $stringBuilder, $stringBuilder.Capacity)
                ...
                # Get process ID
                $processId = [uint32]0
                    [void][LastWarAutoScreenshot.WindowEnumerationAPI]::GetWindowThreadProcessId($hwnd, [ref]$processId)
                ...
                return $true  # Continue enumeration
                    ...
                    Write-LastWarLog -Message "Error processing window $hwnd : $_" -Level Error -FunctionName 'Get-EnumeratedWindows' -Context "Window handle: $hwnd" -LogStackTrace $_
        ...
        # Execute enumeration
        try {
            $enumResult = [LastWarAutoScreenshot.WindowEnumerationAPI]::EnumWindows($enumCallback, [IntPtr]::Zero)
            ...
            Write-LastWarLog -Message "Failed to enumerate windows: $_" -Level Error -FunctionName 'Get-EnumeratedWindows' -Context "EnumWindows API call" -LogStackTrace $_
