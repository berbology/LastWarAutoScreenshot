...
function Select-TargetWindowFromMenu {
    ...
    begin {
        ...
        # Check if we should use internal enumeration (must be inside begin block)
...
function Show-MenuLoop {
    ...
    while ($true) {
        ...
        $activeWindowHandle = [LastWarAutoScreenshot.WindowEnumerationAPI]::GetForegroundWindow()
        # Sort windows
...
function Get-WindowTextLengthInternal {
    param(
        [IntPtr]$WindowHandle
    )
