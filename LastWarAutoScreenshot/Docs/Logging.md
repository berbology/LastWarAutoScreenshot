...
- **Non-Admin Operation:**
  - When running as a non-admin, you may see warnings like:

    > WARNING: Failed to write log entry via EventLog backend: Exception calling "SourceExists" with "1" argument(s): "The source was not found, but some or all event logs could not be searched.  Inaccessible logs: Security."

  - This warning is normal for non-admin users and does not affect logging to the Application log.
  - You only need to run the module or script as Administrator once to create the event log source "LastWarAutoScreenshot".
  - After the source is created, normal (non-admin) operation is fine and logging will work as expected.
  - The warning is from checking if the log source exists, not from creating or writing to the log.
...
```powershell
Import-Module ./src/LastWarAutoClickScreenshot/LastWarAutoScreenshot.psm1
Write-LastWarLog -Message "Test log entry" -ErrorType Info -FunctionName "TestFunction"
```
