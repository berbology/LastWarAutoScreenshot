...
- **Avoid O(n^2) or Worse:** Profile nested loops and recursive calls. Refactor to reduce complexity, but only when I explicitly tell you to refactor.
...
- **Minimize Logging in Hot Paths:** Excessive logging can slow down critical code. Detailed logging is only ok
when it pertains to an issue during troubleshooting. Remind me to remove it once the issue has been fixed
