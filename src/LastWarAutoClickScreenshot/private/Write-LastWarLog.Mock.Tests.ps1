...
describe 'Write-LastWarLog (Mocked)' {
    Context 'Invocation and parameter passing' {
        It 'Should call Write-LastWarLog with correct parameters' {
            ...
            function UnderTest {
                Write-LastWarLog -Message 'Mocked' -Level 'Error' -FunctionName 'TestFunc' -Context 'TestCtx' -LogStackTrace 'Stack' -ForceLog
            }
            UnderTest
            Should -Invoke Write-LastWarLog -ParameterFilter {
                $Message -eq 'Mocked' -and $Level -eq 'Error' -and $FunctionName -eq 'TestFunc' -and $Context -eq 'TestCtx' -and $LogStackTrace -eq 'Stack' -and $ForceLog
            } -Exactly 1
        }
